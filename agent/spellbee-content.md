...
- **class tags**: phonics classification (e.g. `lax:e`, `di:ee`, `tense:a`); used to group questions by shared phonics pattern (and validated against the `known_class_tags` registry). The naming convention and the full catalogue of class tags are in [`spellbee-classes.md`](spellbee-classes.md). Long-story entries leave this empty (they are not grouped by phonics).
