...
This is the puzzle-alerter embedding (a host webview opens the app at `?player=<name>` to make the child solve a few problems, then closes itself). Entered in `bootstrap()` (`common.js`) only when the URL has `?player=<name>` **and** a server-backed data-load hook is present: it sets `bee.is_puzzle = {needs: bee.puzzle_needs}` (spellbee 2, count 3 — small), skips the Choose-player menu, loads the player record (refreshing from the backend when available), and shows a single "Let's earn some coins!" start button. The session has a fixed target of correct answers and ends automatically once met.

- **Countdown:** `add_score()` does `bee.is_puzzle.needs--` only on a non-skipped score (line ~547), so repeats / negative-score-skipped answers do NOT count toward dismissal — only genuine new successes do.
